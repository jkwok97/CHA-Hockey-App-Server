...
const getAllNHLPlayerStats = (req, res) => {
  request(
    `${nhlCOM}/${req.query.playerType}s/${req.query.statType}?cayenneExp=season=${req.query.season}%20and%20gameType=2`,
    (error, response, body) => {
...
const getAllNHLGoalieStats = (req, res) => {
  request(
    `${nhlCOM}/${req.query.playerType}s/${req.query.statType}?cayenneExp=season=${req.query.season}%20and%20gameType=2%20and%20gamesPlayed%20%3E=%206`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
        let leaders = JSON.parse(body);
        let leadersArray = leaders.data;
...
const getAllNHLRookieStats = (req, res) => {
  request(
    `${nhlCOM}/${req.query.playerType}s/${req.query.statType}?cayenneExp=season=${req.query.season}%20and%20gameType=2%20and%20isRookie%20=%20%27Y%27`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
        let leaders = JSON.parse(body);
        let leadersArray = leaders.data;
...
const getAllNhlDefenseStats = (req, res) => {
  request(
    `${nhlCOM}/${req.query.playerType}s/${req.query.statType}?cayenneExp=season=${req.query.season}%20and%20gameType=2%20and%20player.positionCode%20=%20%27D%27`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
        let leaders = JSON.parse(body);
        let leadersArray = leaders.data;
...
const getNhlPlayerStats = (req, res) => {
  request(
    `https://statsapi.web.nhl.com/api/v1/people/${req.query.id}/stats?stats=statsSingleSeason&season=20212022`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
...
const getOnPaceNhlPlayerStats = (req, res) => {
  request(
    `https://statsapi.web.nhl.com/api/v1/people/${req.query.id}/stats?stats=onPaceRegularSeason&season=20212022`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
        var info = JSON.parse(body);
        res.send(info);
...
const getNhlPlayerInfo = (req, res) => {
  request(
    `https://statsapi.web.nhl.com/api/v1/people/${req.params.id}?expand=person.stats&stats=yearByYear,yearByYearPlayoffs,careerRegularSeason&expand=stats.team&site=en_nhlCA`,
    (error, response, body) => {
      if (!error && response.statusCode == 200) {
    ...
    (error, response) => {
      if (!error && response.statusCode == 200) {
        const result = {
          statusCode: 200,
          message: "Request Success",
          data: response.body.data.player_stats,
        };
...
const getStats = (info) => {
  ...
  const playerStats = s;
  const p = playerStats.filter(
    (stat) => stat["league"]["name"] === "National Hockey League"
  );

  return playerType.primaryPosition.code === "G"
    ? extractGoalieStats(p, totals)
    : extractPlayerStats(p, totals);
};
...
module.exports = {
  getAllNHLPlayerStats,
  getAllNHLGoalieStats,
  getNHLPlayerSummary,
  getAllNHLRookieStats,
  getAllNHLRookieSummary,
  getNhlPlayerStats,
  getOnPaceNhlPlayerStats,
  getCareerNHLPlayerStats,
  getNhlPlayerInfo,
  getAllNhlDefenseStats,
  getNHLStatsSportsNet,
};
